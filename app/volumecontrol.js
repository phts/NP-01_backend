...
function CoreVolumeController(commandRouter) {
  ...
  var outputdevicename = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'outputdevicename'
  ...
  if (process.env.MODULAR_ALSA_PIPELINE === 'true') {
    ...
    if (device === 'softvolume') {
      device = this.commandRouter.executeOnPlugin(
        'audio_interface',
        'alsa_controller',
        'getConfigParam',
        'softvolumenumber'
  ...
  volumecurve = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'volumecurvemode'
  )
  volumesteps = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'volumesteps'
  ...
  var checkAndReportAlsaVolume = function () {
    ...
    getInfo(function (err, result) {
      ...
      if (err) {
        ...
        self.logger.info(
          'VolumeController:: External volume change detected: vol=' + result.volume + ' mute=' + result.muted
        )
    ...
    if (volumescript.enabled) {
      try {
        ...
        if (volumescript.setmutescript !== undefined && volumescript.setmutescript.length > 0) {
          ...
          if (val === 0) {
            Volume.mute = true
            scriptmute = execSync('/bin/sh ' + volumescript.setmutescript + ' 1', {
              uid: 1000,
              gid: 1000,
              encoding: 'utf8',
            })
            ...
            scriptmute = execSync('/bin/sh ' + volumescript.setmutescript + ' 0', {
              uid: 1000,
              gid: 1000,
              encoding: 'utf8',
            })
      ...
      if (volumecurve === 'logarithmic') {
        ...
        if (
          devicename === 'PianoDACPlus' ||
          devicename === 'Allo Piano 2.1' ||
          devicename === 'PianoDACPlus multicodec-0'
        ) {
  ...
  self.syncAlsaMonitor = function () {
    ...
    if (shouldMonitorAlsa) {
      alsaRestartDelay = 5000
      if (!alsaMonitorProcess) monitorAlsaEvents()
      else {
...
CoreVolumeController.prototype.updateVolumeSettings = function (data) {
  ...
  self.logger.info(
    'Updating Volume Controller Parameters: Device: ' +
      data.device +
      ' Name: ' +
      data.name +
      ' Mixer: ' +
  ...
  if (
    data.mixertype !== undefined &&
    mixertype !== 'None' &&
    mixer !== undefined &&
    mixer.length &&
    (data.mixertype === 'None' || data.mixertype === 'Software')
  ) {
  ...
  if (
    ...
    mixertype === 'None' &&
    data.mixertype !== undefined &&
    (data.mixertype === 'Software' || data.mixertype === 'Hardware')
  ) {
    setTimeout(() => {
...
CoreVolumeController.prototype.retrievevolume = function () {
  ...
  if (volumeOverride) {
    ...
    if (mixertype === 'None') {
      ...
      Volume.mute = false
      Volume.disableVolumeControl = true
      return libQ.resolve(Volume).then(function (Volume) {
        defer.resolve(Volume)
        self.commandRouter.volumioupdatevolume(Volume)
      })
    } else if (mixertype === 'Software') {
      exec(
        '/usr/bin/amixer -M get -c ' +
          device +
          " 'SoftMaster' | awk '$0~/%/{print}' | cut -d '[' -f2 | tr -d '[]%' | head -1",
...
CoreVolumeController.prototype.setStartupVolume = function () {
  ...
  var startupVolume = this.commandRouter.executeOnPlugin(
    'audio_interface',
    'alsa_controller',
    'getConfigParam',
    'volumestart'
