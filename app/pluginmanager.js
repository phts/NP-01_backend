...
function PluginManager(ccommand, server) {
  ...
  if (fs.existsSync(myVolumioPMPath)) {
    ...
    self.myVolumioPluginManager = new (require(myVolumioPMPath))(
      self.coreCommand,
        ...
        if (!plugins[i].volumioStart) {
          self.logger.error(
            'Plugin ' +
              ...
              ' ' +
              plugins[i].name +
              " failed to complete 'onVolumioStart' in a timely fashion"
          )
        }
      }
            ...
            if (status != 'STARTED') {
              self.logger.error(
                'Plugin ' +
                  ...
                  ' ' +
                  plugins[i].name +
                  " failed to complete 'onStart' in a timely fashion"
              )
            }
          }
...
PluginManager.prototype.loadCorePlugin = function (folder) {
  ...
  if (shallStartup == true) {
    ...
    var context = new (require(__dirname + '/pluginContext.js'))(
      self.coreCommand,
      self.websocketServer,
      self.configManager
    )
    ...
    try {
      ...
      self.logger.error('!!!! WARNING !!!!')
      self.coreCommand.pushToastMessage(
        'error',
        name + ' Plugin',
        self.coreCommand.getI18nString('PLUGINS.PLUGIN_START_ERROR')
      )
    ...
    if (pluginInstance && pluginInstance.onVolumioStart !== undefined) {
      ...
      if (Object.prototype.toString.call(myPromise) != Object.prototype.toString.call(libQ.resolve())) {
        ...
        self.logger.error(
          'ATTENTION!!!: Plugin ' + name + ' does not return adequate promise from onVolumioStart: please update!'
        )
...
PluginManager.prototype.isEnabled = function (category, pluginName) {
  ...
  if (isEnabled === undefined) {
    if (
      self.myVolumioPluginManager !== undefined &&
      self.myVolumioPluginManager.checkIfPluginIsInCurrentPlan(category, pluginName)
    ) {
...
PluginManager.prototype.getConfigurationFile = function (context, fileName) {
  var self = this
  return (
    S(self.configurationFolder).ensureRight('/').s +
...
PluginManager.prototype.installPlugin = function (url) {
  ...
  self.pushMessage('installPluginStatus', {
    progress: 10,
    message: self.coreCommand.getI18nString('PLUGINS.DOWNLOADING_PLUGIN'),
    title: modaltitle,
  ...
  exec(downloadCommand, function (error, stdout, stderr) {
    if (error !== null) {
      ...
      self
        ...
        .then(
          self.pushMessage.bind(self, 'installPluginStatus', {
            progress: 30,
        ...
        .then(function (folder) {
          ...
          currentMessage =
            name +
            ' ' +
            self.coreCommand.getI18nString('PLUGINS.SUCCESSFULLY_INSTALLED') +
            ', ' +
            self.coreCommand.getI18nString('PLUGINS.ENABLE_PLUGIN_NOW_QUESTION')
          var enablePayload = {name: package_json.name, category: category, action: 'enable'}
          var buttons = [
            {name: self.coreCommand.getI18nString('COMMON.CLOSE'), class: 'btn btn-warning'},
            {
              name: self.coreCommand.getI18nString('PLUGINS.ENABLE_PLUGIN'),
              class: 'btn btn-info',
              emit: 'pluginManager',
          ...
          self.pushMessage('installPluginStatus', {
            progress: 100,
            message: currentMessage,
            title: self.coreCommand.getI18nString('PLUGINS.SUCCESSFULLY_INSTALLED'),
            advancedLog: advancedlog,
            buttons: buttons,
          })
          return folder
    ...
    if (error !== null) {
      ...
      self
        ...
        .then(function (e) {
          self.pushMessage('installPluginStatus', {
            progress: 20,
            message: self.coreCommand.getI18nString('PLUGINS.PLUGIN_STOPPED'),
            title: modaltitle,
          })
          return e
        })
        ...
        .then(function (folder) {
          ...
          self.pushMessage('installPluginStatus', {
            progress: 100,
            message: currentMessage,
            title: self.coreCommand.getI18nString('PLUGINS.PLUGIN_UPDATE_COMPLETED'),
            advancedLog: advancedlog,
            buttons: [{name: self.coreCommand.getI18nString('COMMON.CLOSE'), class: 'btn btn-warning'}],
          })
          return folder
...
PluginManager.prototype.checkPluginDependencies = function (folder) {
  ...
  try {
    var native_modules = execSync(
      `find ${folder}/node_modules -name obj.target -prune -false -o -type f -name "*.node" 2>/dev/null`,
      {encoding: 'utf8'}
    )
      .split('\n')
      .filter((n) => n)
  ...
  if (package_json.engines) {
    pendingResult = pendingResult.then((result) => {
      if (package_json.engines.node) {
        ...
        if (nodeVersion === null) {
          ...
          self.logger.warn(
            `Plugin ${package_json.name} has native modules version miss match! Current ${nodeVersion.major} != plugin ${pluginNodeVersion.major} effected modules:`,
            native_modules
          )
          result.success = 'failed'
      ...
      if (package_json.engines.volumio) {
        ...
        return self.coreCommand.getSystemVersion().then((e) => {
  ...
  return pendingResult.then((result) => {
    if (result.success === 'failed') {
      result.message =
        'Plugin failed the dependency check ' +
        result.message +
...
PluginManager.prototype.executeInstallationScript = function (folder) {
  ...
  fs.stat(installScript, function (err, stat) {
    if (err) {
      ...
      exec(
        'echo volumio | sudo -S sh ' + installScript + ' > /tmp/installog',
        {uid: 1000, gid: 1000, maxBuffer: 2024000},
  ...
  if (self.config.has(key)) {
    ...
    self
      ...
      .then(function (e) {
        self.pushMessage('installPluginStatus', {
          progress: 60,
          message: self.coreCommand.getI18nString('PLUGINS.PLUGIN_DISABLED'),
          title: modaltitle,
        })
        return e
      })
      ...
      .then(function (e) {
        self.pushMessage('installPluginStatus', {
          progress: 70,
          message: self.coreCommand.getI18nString('PLUGINS.REMOVING_NECESSARY_UTILITIES'),
          title: modaltitle,
        })
        return e
      })
      ...
      .then(function (e) {
        self.pushMessage('installPluginStatus', {
          progress: 90,
          message: self.coreCommand.getI18nString('PLUGINS.FINALIZING_UNINSTALL'),
          title: modaltitle,
        })
        return e
      })
      ...
      .then(function (e) {
        self.pushMessage('installPluginStatus', {
          progress: 100,
          message: self.coreCommand.getI18nString('PLUGINS.PLUGIN_UNINSTALLED'),
          title: modaltitle,
          buttons: [{name: self.coreCommand.getI18nString('COMMON.CLOSE'), class: 'btn btn-warning'}],
        })
        return e
      ...
      .fail(function (e) {
        self.pushMessage('installPluginStatus', {
          progress: 100,
          message: self.coreCommand.getI18nString('PLUGINS.PLUGIN_UNINSTALL_ERROR_OCCURRED') + ': ' + e,
          title: self.coreCommand.getI18nString('PLUGINS.PLUGIN_UNINSTALL_FAILED'),
          buttons: [{name: self.coreCommand.getI18nString('COMMON.CLOSE'), class: 'btn btn-warning'}],
        })
        defer.reject(new Error())
      })
  ...
  if (installed != undefined) {
    installed.then(function (installedPlugins) {
      for (var e = 0; e < installedPlugins.length; e++) {
        var pluginpretty = {
          prettyName: installedPlugins[e].prettyName,
          version: installedPlugins[e].version,
          category: installedPlugins[e].category,
        }
  ...
  function pushAvailablePlugins(response) {
    for (var i = 0; i < response.categories.length; i++) {
      ...
      for (var a = 0; a < plugins.length; a++) {
        ...
        if (fs.existsSync('/data/testplugins')) {
          thisPlugin.url =
            'https://plugins.volumio.workers.dev/pluginsv2/downloadLatest/' +
            plugins[a].name +
            '/' +
            variant +
            '/' +
        ...
        version.variants.forEach((versionVariant) => {
          if (versionVariant.variant === variant + '/' + os + '/' + arch) {
            responseData.buttons.push({
              ...
              payload: {
                url:
                  'https://plugins.volumio.workers.dev/pluginsv2/download/' +
                  data.name +
                  '/' +
                  version.version +
                  '/' +
...
PluginManager.prototype.getPrettyName = function (package_json) {
  if (package_json.volumio_info !== undefined && package_json.volumio_info.prettyName !== undefined) {
  ...
  for (var i in categories) {
    ...
    for (var k in plugins) {
      ...
      if (plugin_exists == false) {
        self.logger.info(
          'Configured plugin ' + category + '/' + plugin + ' cannot be loaded. Removing from configuration'
        )
