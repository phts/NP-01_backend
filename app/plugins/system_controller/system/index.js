  ...
  self.commandRouter
    ...
    .then(function (uiconf) {
      ...
      self.configManager.setUIConfigParam(
        uiconf,
        'sections[0].content[3].value.label',
        self.getLabelForSelect(
          self.configManager.getValue(uiconf, 'sections[0].content[3].options'),
            ...
            .then(function (result) {
              if (result.available.length > 0) {
                ...
                for (var i in disklist) {
                  ...
                  var description =
                    self.commandRouter.getI18nString('SYSTEM.INSTALL_TO_DISK_DESC') +
                    ': ' +
                    device.name +
                    ' ' +
                    self.commandRouter.getI18nString('SYSTEM.INSTALL_TO_DISK_SIZE') +
                    ': ' +
                    device.size
                  var title = self.commandRouter.getI18nString('SYSTEM.INSTALL_TO_DISK_DESC') + ' ' + device.name
                  var message =
                    self.commandRouter.getI18nString('SYSTEM.INSTALL_TO_DISK_MESSAGE') +
                    ' ' +
                    device.name +
                    ' ' +
                    device.size +
                    '. ' +
                    self.commandRouter.getI18nString('SYSTEM.INSTALL_TO_DISK_MESSAGE_WARNING')
                  var onClick = {
                    type: 'emit',
                    message: 'installToDisk',
                    data: {from: result.current, target: device.device, hwdevice: hwdevice},
                    askForConfirm: {title: title, message: message},
                  }
                  var item = {
      ...
      self.commandRouter
        ...
        .then(function (languageSelector) {
          ...
          var language_code = self.commandRouter.executeOnPlugin(
            'miscellanea',
            'appearance',
            'getConfigParam',
            'language_code'
...
ControllerSystem.prototype.setHostname = function (hostname) {
  ...
  fs.writeFile('/etc/hostname', newhostname, function (err) {
    if (err) {
      ...
      self.commandRouter.pushToastMessage(
        'alert',
        self.commandRouter.getI18nString('SYSTEM.SYSTEM_NAME'),
        ...
        fs.writeFile('/etc/hosts', '127.0.0.1       localhost ' + newhostname, function (err) {
          if (err) {
            ...
            self.commandRouter.pushToastMessage(
              'success',
              self.commandRouter.getI18nString('SYSTEM.SYSTEM_NAME'),
              self.commandRouter.getI18nString('SYSTEM.SYSTEM_NAME_NOW') + ' ' + hostname
            )
            self.logger.info('Hostname now is ' + newhostname)
            var avahiconf =
              '<?xml version="1.0" standalone="no"?><service-group><name replace-wildcards="yes">' +
              hostname +
              '</name><service><type>_http._tcp</type><port>80</port></service></service-group>'
            exec(
              '/usr/bin/sudo /bin/chmod -R 777 /etc/avahi/services/',
              {uid: 1000, gid: 1000},
...
ControllerSystem.prototype.restartAvahi = function () {
  ...
  exec(
    '/usr/bin/sudo /bin/systemctl restart avahi-daemon.service',
    {
      uid: 1000,
      gid: 1000,
    },
...
ControllerSystem.prototype.getSystemInfo = function () {
  ...
  var thisDeviceStatus = self.commandRouter.executeOnPlugin(
    'system_controller',
    'volumiodiscovery',
    'getThisDevice',
    ''
...
ControllerSystem.prototype.deviceDetect = function (data) {
  ...
  info.then(function (infos) {
    if (
      infos != undefined &&
      infos.hardware != undefined &&
      (infos.hardware === 'x86' || infos.hardware === 'x86_amd64' || infos.hardware === 'x86_i386')
    ) {
      device = 'x86'
...
ControllerSystem.prototype.callHome = function () {
  ...
  info.then(function (infos) {
    if (infos.variant && infos.systemversion && infos.hardware && md5) {
      ...
      exec(
        '/usr/bin/curl -X POST --data-binary "device=' +
          infos.hardware +
          '&variante=' +
          infos.variant +
          '&version=' +
          ...
          '&uuid=' +
          md5 +
          '" http://updates.volumio.org/downloader-v1/track-device',
        function (error, stdout, stderr) {
          if (error !== null) {
            if (calltrials < 3) {
              setTimeout(function () {
...
ControllerSystem.prototype.enableSSH = function (data) {
  ...
  exec(
    '/usr/bin/sudo /bin/systemctl ' +
      immediate +
      ' ssh.service && /usr/bin/sudo /bin/systemctl ' +
      action +
      ' ssh.service',
    {uid: 1000, gid: 1000},
    function (error, stdout, stderr) {
...
ControllerSystem.prototype.getDisks = function () {
  ...
  var currentdiskRaw = execSync('/bin/mount | grep "/imgpart" | head -n 1 | cut -d " " -f 1', {
    uid: 1000,
    gid: 1000,
    encoding: 'utf8',
  })
  ...
  if (currentdisk === 'mmcblkp' || currentdisk === 'nvmenp') {
    currentdiskRaw = execSync(
      '/bin/mount | grep "/imgpart" | head -n 1 | cut -d " " -f 1 | cut -d "/" -f 3 | cut -d "p" -f 1',
  ...
  for (var i = 0; i < disks.length; i++) {
    if (
      disks[i].indexOf(currentdisk) >= 0 ||
      disks[i].indexOf('loop') >= 0 ||
      disks[i].indexOf('rpmb') >= 0 ||
      disks[i].indexOf('boot') >= 0
    ) {
    } else {
...
ControllerSystem.prototype.installToDisk = function (data) {
  ...
  if (hwdevice !== 'x86' && hwdevice !== 'Raspberry PI') {
    ...
    if (Number(ddsizeRaw) > Number(ddsizeRawDest)) {
      ...
      try {
        var copy = exec(
          '/usr/bin/sudo /usr/bin/dcfldd if=' +
            source +
            ' of=' +
            target +
            ' bs=1M status=on sizeprobe=if statusinterval=10 >> /tmp/install_progress 2>&1',
          {uid: 1000, gid: 1000, encoding: 'utf8'}
        )
      } catch (e) {
        error = true
...
ControllerSystem.prototype.notifyInstallToDiskStatus = function (data) {
  ...
  if (status === 'started') {
    ...
    responseData.message =
      self.commandRouter.getI18nString('SYSTEM.INSTALLING_TO_DISK_ERROR_MESSAGE') + ': ' + data.error
  }
...
ControllerSystem.prototype.getExperienceAdvancedSettings = function () {
  ...
  var responseObject = {
    options: [
      {id: false, label: simpleSettingsString},
      {id: true, label: fullSettingsString},
    ],
...
ControllerSystem.prototype.initializeFirstStart = function () {
  ...
  if (isFirstStart) {
    ...
    if (process.env.AUTO_RENAME_HOTSPOT_TO_UID_ONFIRSTSTART === 'true') {
      ...
      self.commandRouter.executeOnPlugin('system_controller', 'network', 'saveHotspotSettings', hotspotOptions)
    }
...
ControllerSystem.prototype.setTimezone = function (data) {
  ...
  try {
    ...
    execSync('/usr/bin/sudo /bin/ln -s /usr/share/zoneinfo/' + data + ' /etc/localtime', {
      uid: 1000,
      gid: 1000,
      encoding: 'utf8',
    })
  ...
  try {
    ...
    try {
      ...
      setTimeout(() => {
        execSync("/usr/bin/sudo /usr/bin/timedatectl set-timezone '" + data + "'", {
          uid: 1000,
          gid: 1000,
          encoding: 'utf8',
        })
...
ControllerSystem.prototype.setLanguageTimezone = function (data) {
  ...
  return self.commandRouter.executeOnPlugin('miscellanea', 'appearance', 'setLanguage', data)
}
