...
ControllerMpd.prototype.restartMpd = function (callback) {
  ...
  if (callback) {
    exec('/usr/bin/sudo /bin/systemctl restart mpd.service ', {uid: 1000, gid: 1000}, function (error, stdout, stderr) {
      self.mpdEstablish()
      setTimeout(() => {
...
ControllerMpd.prototype.createMPDFile = function (callback) {
  ...
  exec('/usr/bin/sudo /bin/chmod 777 /etc/mpd.conf', {uid: 1000, gid: 1000}, function (error, stdout, stderr) {
    if (error != null) {
