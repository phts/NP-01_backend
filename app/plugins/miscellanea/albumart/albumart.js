...
try {
  variant = execSync("cat /etc/os-release | grep ^VOLUMIO_VARIANT | tr -d 'VOLUMIO_VARIANT=\"'")
    .toString()
    .replace('\n', '')
} catch (e) {
  variant = 'none'
...
var retrieveAlbumart = function (artist, album, size, cb) {
  ...
  if (album === null) {
    ...
    var url =
      'https://meta.volumio.org' +
      encodeURI('/metas/v1/getDatas?mode=artistArt&artist=' + artist + '&variant=' + variant)
    https
    ...
    var options = {
      ...
      port: 80,
      path: encodeURI(
        '/2.0/?format=json&api_key=' + apiKey + '&method=' + method + '.getinfo&artist=' + artist + '&album=' + album
      ),
    }
    http
