...
AlbumArt.prototype.saveAlbumartOptions = function (data) {
  ...
  self.commandRouter.pushToastMessage(
    'success',
    self.commandRouter.getI18nString('APPEARANCE.ALBUMART_SETTINGS'),
...
AlbumArt.prototype.clearAlbumartCache = function () {
  ...
  exec(
    '/bin/rm -rf /data/albumart/folder/* /data/albumart/metadata/* /data/albumart/web/*',
    {uid: 1000, gid: 1000},
    function (error, stdout, stderr) {
      if (error) {
        ...
        self.commandRouter.pushToastMessage(
          'error',
          self.commandRouter.getI18nString('APPEARANCE.ALBUMART_SETTINGS'),
          self.commandRouter.getI18nString('APPEARANCE.ALBUMART_CACHE_CLEAR_ERROR')
        )
      } else {
        cacheid++
        self.config.set('cacheid', cacheid)
        ...
        self.commandRouter.pushToastMessage(
          'success',
          self.commandRouter.getI18nString('APPEARANCE.ALBUMART_SETTINGS'),
          self.commandRouter.getI18nString('APPEARANCE.ALBUMART_CACHE_CLEARED')
        )
      }
    }
  )
